...
Example: <@97153790897045504> | `97153790897045504`"""
        channel = self.bot.get_channel(1299891459255959644)  # Grabs the error reporting channel for Mudbot on
        # my private server.
...
(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    [str(guild.id), 2, "(", ")", 1, 300, 1, 30, str(exp_thresholds), str(tier_thresholds),
                     1, "{}", "{}", "{}", 1, 1, "{}", "{}", 1, "{}", 0, 0])
        con.commit()
        con.close()
